...
import { getAcceptedUsersByDocumentApi } from '../../../../../shared/api/admin/legal.api.js';

const AcceptedUsers = ({ isOpen, onClose, documentId }) => {
    ...
    const loadData = async () => {
        ...
        setError(null);
        try {
            const response = await getAcceptedUsersByDocumentApi(documentId, {
            ...
            setError(err.message || 'Failed to load accepted users');
            console.error('Load accepted users error:', err);
        } finally {
            setIsLoading(false);
        }
    };
        ...
        <div style={styles.overlay} onClick={onClose}>
            <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
                <div style={styles.header}>
                    <h3 style={styles.title}>✅ Accepted Users</h3>
                ...
                {isLoading ? (
                    ...
                    <div style={styles.content}>
                                        ...
                                        <tbody>
                                            {data.users.map(user => (
                                                <tr key={user.user_id}>
                                                    ...
                                                    <td>{user.accepted_at ? new Date(user.accepted_at).toLocaleString() : 'N/A'}</td>
                                                    <td>{user.signature_name || user.signature_type || 'N/A'}</td>
                                                    <td>{user.device || user.platform || 'N/A'}</td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
                                        ...
                                        <tbody>
                                            {data.experts.map(expert => (
                                                <tr key={expert.expert_id}>
                                                    ...
                                                    <td>{expert.accepted_at ? new Date(expert.accepted_at).toLocaleString() : 'N/A'}</td>
                                                    <td>{expert.signature_name || expert.signature_type || 'N/A'}</td>
                                                    <td>{expert.device || expert.platform || 'N/A'}</td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
                        ...
                        {(!data.users || data.users.length === 0) && (!data.experts || data.experts.length === 0) && (
                            <div style={styles.empty}>No users have accepted this document yet</div>
                        )}
