  ...
  const handleSendReply = async (e) => {
    ...
    try {
      ...
      if (res.ok && data.data) {
        ...
        setReplyText("");
        setInquiries(prev => prev.map(item => item.id === selectedInquiry.id ? { ...item, status: "expert_replied", message: replyText } : item));
      } else {
  ...
  const handleRejectInquiry = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setInquiries(prev => prev.map(item => item.id === selectedInquiry.id ? { ...item, status: "rejected" } : item));
      }
    } catch (err) {
      console.error(err);
    }
  ...
  const handleConvertInquiry = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setInquiries(prev => prev.map(item => item.id === selectedInquiry.id ? { ...item, status: "converted" } : item));
          ...
          <InquiryList>
            {listLoading ? (
              ...
              <EmptyState>
                ...
                <p style={{ margin: 0, fontSize: "0.9rem" }}>No inquiries received yet.</p>
              </EmptyState>
            ) : (
          ...
          {selectedInquiry ? (
            <>
              <MobileBackBar>
                ...
                <div style={{ fontWeight: 600, fontSize: "0.95rem", color: "#191919" }}>{selectedInquiry.user_name_snapshot}</div>
              </MobileBackBar>

              <DetailHeader>
                <DetailTitleBlock>
                  <SubjectTitle>{selectedInquiry.subject}</SubjectTitle>
                  <div style={{ fontSize: "0.85rem", color: "#5e5e5e" }}>
                    From: <strong>{selectedInquiry.user_name_snapshot}</strong>
                  </div>
                </DetailTitleBlock>
                <ActionHeaderBlock>
                  {selectedInquiry.status !== "converted" && selectedInquiry.status !== "closed" && (
                    <HeaderBtn $primary onClick={handleConvertInquiry}>Convert</HeaderBtn>
                  )}
                  {selectedInquiry.status !== "rejected" && selectedInquiry.status !== "closed" && (
                    <HeaderBtn onClick={handleRejectInquiry}>Reject</HeaderBtn>
                  )}
                  {selectedInquiry.status !== "closed" && (
                    <HeaderBtn onClick={handleCloseInquiry}>Close</HeaderBtn>
                  )}
                </ActionHeaderBlock>
              </DetailHeader>

              <InfoBox>
                <div style={{ fontWeight: 600, color: "#191919", marginBottom: "4px" }}>Inquiry Details</div>
                <InfoGrid>
                  <InfoItem>Preferred Contact: <span>{selectedInquiry.preferred_contact_method}</span></InfoItem>
                  <InfoItem>Preferred Time: <span>{selectedInquiry.preferred_contact_time}</span></InfoItem>
                  <InfoItem>Email: <span>{selectedInquiry.user_email_snapshot || "N/A"}</span></InfoItem>
                  <InfoItem>Phone: <span>{selectedInquiry.user_mobile_snapshot || "N/A"}</span></InfoItem>
                </InfoGrid>
                {selectedInquiry.user_email_snapshot?.includes("***") && (
                  <div style={{ fontSize: "0.75rem", color: "#e65100", marginTop: "8px", display: "flex", alignItems: "center", gap: "4px" }}>
                    <FiInfo size={12} /> Contact details are masked. Upgrade plan or consult with client to unlock contact info.
                  </div>
                )}
              </InfoBox>
                  ...
                  return (
                    <MessageGroup key={msg.id} $isMe={isMe}>
                      <MessageBubble $isMe={isMe}>
                        <MessageSender>{isMe ? "You" : selectedInquiry.user_name_snapshot}</MessageSender>
              ...
              <InputBar onSubmit={handleSendReply}>
                <ReplyInput
                  ...
                  placeholder={selectedInquiry.status === "closed" || selectedInquiry.status === "rejected" ? "Inquiry is inactive" : "Write a reply..."}
                  disabled={selectedInquiry.status === "closed" || selectedInquiry.status === "rejected" || loading}
                />
                <SendButton type="submit" disabled={selectedInquiry.status === "closed" || selectedInquiry.status === "rejected" || !replyText.trim() || loading}>
                  <FiSend size={16} />
                </SendButton>
              </InputBar>
            </>
